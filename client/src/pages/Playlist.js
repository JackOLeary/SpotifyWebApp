...
const Playlist = () => {
    ...
    const tracksWithAudioFeatures = useMemo(() => {
        ...
        return tracks.map(({ track }) => {
            // set trackToAdd to track object
            const trackToAdd = track;
            // if we haven't added audio features then add them by looking at track id
            ...
            <main>
                <SectionWrapper title="Playlist" breadcrumb="true">
                    <StyledDropdown active={!!sortValue}>
                        ...
                        {/* update UseState for current sorted value */}
                        <select name="track-order" id="order-select" onChange={e => setSortValue(e.target.value)}>
                            <option value="">Sort tracks</option>
                            {/* Map sortOptions array to option drop down list */}
                            {sortOptions.map((option, i) => (
                                <option value={option} key={i}>
                                    {/* uppercase for first letter of each option */}
